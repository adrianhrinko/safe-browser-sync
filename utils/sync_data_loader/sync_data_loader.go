...
import (
	...
	"github.com/brave/go-sync/sync_chain_specifics"
	"github.com/brave/go-sync/utils"
	"github.com/brave/go-sync/utils/safe_browser_config"
...
func getEncryptedData(msg []byte, syncSpecs *sync_chain_specifics.SyncChainSpecifics) (*sync_pb.EncryptedData, error) {

	encryptedData := &sync_pb.EncryptedData{}

	encrypted, err := utils.AESCBCEncrypt(syncSpecs.encKey, msg, syncSpecs.hmacKey)
	if err != nil {
...
func updateNigoriWithEncryptionData(syncSpecs *sync_chain_specifics.SyncChainSpecifics) (*sync_pb.ClientToServerResponse, error) {
	...
	keyBag := &sync_pb.NigoriKeyBag{
		Key: []*sync_pb.NigoriKey{
			{
				DeprecatedName:    aws.String(syncSpecs.key_name),
				DeprecatedUserKey: zeroez,
				EncryptionKey:     syncSpecs.enc_key,
				MacKey:            syncSpecs.hmac_key,
			},
		},
	}
...
func preparePrefsEntity(prefName string, pref interface{},
	syncSpecs *sync_chain_specifics.SyncChainSpecifics) (*sync_pb.SyncEntity, error) {
...
func loadSettings(settings *safe_browser_config.SafeBrowserSettings,
	syncSpecs *sync_chain_specifics.SyncChainSpecifics) (*sync_pb.ClientToServerResponse, error) {
...
func createAndInitSpecs(syncServerURL string) sync_chain_specifics.SyncChainSpecifics {
	syncSpecs, err := sync_chain_specifics.New(&http.Client{}, syncServerURL)
	check(err)
...
func saveDataOnDisc(syncSpecs *sync_chain_specifics.SyncChainSpecifics) {
	err := syncSpecs.saveToFile("sync_specs.json")
	check(err)
	err = syncSpecs.saveSyncMnemonicToFile("mnemonic.txt")
	check(err)
	err = syncSpecs.saveSyncQRCodeToFile("qr.json")
	check(err)
}

func getPassHash(password string) (string, error) {

	salt := utils.GenRandomBytes(32)
	hash, err := utils.Scrypt([]byte(password), salt)
	if err != nil {
		return "", err
	}
	...
	return b64.StdEncoding.EncodeToString(passHash), nil
}

func createSyncData(settings *safe_browser_config.SafeBrowserConfig) {
...
func main() {
	...
	settings := &safe_browser_config.SafeBrowserSettings{
